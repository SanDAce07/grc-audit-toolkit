# IT Audit Report

**Report Date:** _______________  
**Audit Period:** _______________  
**Prepared By:** _______________  
**Distribution:** Restricted — Management Only

---

## Executive Summary

This report presents the results of the IT General Controls (ITGC) review for [Entity Name] for the period [start] through [end]. Document the applicable professional standard, internal methodology, and engagement scope here; remove this sentence if no external standard governs the work.

**Overall Conclusion:** ☐ Effective ☐ Partially Effective ☐ Ineffective

---

## Scope

The following systems and control areas were in scope:

- **Systems:** [ERP, Financial Reporting System, etc.]
- **Control Areas:** Access to Programs & Data, Change Management, Computer Operations, Program Development

---

## Findings Summary

| Finding ID | Control Area | Severity | Description | Management Response | Due Date |
|-----------|-------------|----------|-------------|-------------------|---------|
| F-001 | | ☐ Critical ☐ High ☐ Medium ☐ Low | | | |
| F-002 | | ☐ Critical ☐ High ☐ Medium ☐ Low | | | |

---

## Detailed Findings

### Finding F-001: [Title]

**Severity:** High  
**Control Area:** Access to Programs & Data  
**Criteria:** [Applicable standard/framework]  
**Condition:** [What was observed]  
**Cause:** [Root cause]  
**Effect:** [Business impact / risk]  
**Recommendation:** [Suggested remediation]  
**Management Response:** [Client response]  
**Target Date:** _______________

---

## Conclusion

[Summary paragraph on overall control environment effectiveness]

---

*This report is intended solely for the use of management and should not be distributed to third parties without prior written consent.*
